...
jugarTateti:- restartTateti,write('Bienvenido al Gatos invensible, '),nl,
write('para jugar ingrese :-tic(numero).'),nl,
write('Ingresa el nombre del jugador'),nl,read(_nombre),nl,
nb_setval('nombre', _nombre),
write('Ingresa el simbolo que desea utilizar 1 - "O", 2 - "X"'),nl,read(Simbolo),nl,
( Simbolo = 1 -> 
	nb_setval('jugador', 'O'),
	nb_setval('PC', 'X')
	; Simbolo = 2 -> 
	nb_setval('jugador', 'X'),
	nb_setval('PC', 'O')
	; Simbolo > 3 -> jugarTateti
	; Simbolo < 1 -> jugarTateti
),
write('¿Quien inicia primero? 0 - Jugador, 1 - PC, 2 - Aleatorio'),nl,read(_turno),nl,
(
	_turno < 0 -> jugarTateti
	; _turno > 2 -> jugarTateti
	; A is _turno
),
		(comienza(A),
		(modo(X);true),
...
tic(X):- nb_getval('jugador', A),not(tateti),box(X,Y),not(Y = 'X'),not(Y = 'O'), retract(box(X,_)), assert(box(X, A)),
		not(tateti),(automaticPlay),tablero,!.

tic(X):- nb_getval('nombre', A), tateti->(tablero,nl,write('Fin de la partida, suerte de principiante...'),nl,write('El ganador es el jugador '), write(A), write(' A ganado'),nl);nb_getval('nombre', A),not(jugadaPosible),write('Fin de la partida. Empate. '),nl,write('El jugador '),write(A),write(' y la PC han empatado'),nl,tablero,!;(write('El casillero esta� ocupado'),nl,fail).

iaTic(X):- nb_getval('PC', A), not(tateti),box(X,Y),not(Y = 'X'),not(Y = 'O'), retract(box(X,_)), assert(box(X, A)),
		not(tateti),nl, jugadaPosible, write('Tu turno'),nl,!.

iaTic(X):-nb_getval('nombre', A), tateti->(nl,write('Fin de la partida'),nl, write('El jugador '),write(A), write(' a perdido'),nl);not(jugadaPosible),nl,fail.
